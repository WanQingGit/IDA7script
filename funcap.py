...
class FunCapHook(ida_dbg.DBG_Hooks):
    """
    ...
    def off(self):
        ...
        if self.out is not None:
            self.out.close()
        if self.strings_out is not None:
    ...
    def get_num_args_stack(self, addr):
        ...
        argFrameSize = idc.GetStrucSize(idc.GetFrame(addr)) - idc.GetFrameSize(addr) + idc.GetFrameArgsSize(addr)
    ...
    def format_normal(self, regs):
        ...
        for reg in regs:
            ...
            function_name = idc.GetFuncOffset(prev_memval)  # no more dereferencing. is this a function ?
            if (function_name):
    ...
    def dereference(self, address, size):
        return idc.GetManyBytes(address, size, use_dbg=True)
    ...
    def handle_function_start(self, ea):
        ...
        raw_context = self.get_context(ea=ea)
        if self.colors:
            idc.SetColor(ea, idc.CIC_FUNC, self.FUNC_COLOR)
    ...
    def handle_call(self, ea):
        ...
        if self.colors:
            idc.SetColor(ea, idc.CIC_ITEM, self.CALL_COLOR)
    ...
    def handle_after_jump(self, ea):
        ...
        if self.comments:
            idc.MakeComm(self.current_caller['addr'], "0x%x" % ea)
        seg_name = idc.SegName(ea)
        if self.code_discovery and (not idc.isCode(idc.GetFlags(ea)) or not self.isCode) and not self.is_system_lib(
                seg_name):
            ...
            if not idc.MakeCode(ea):
                ins = idc.DecodeInstruction(ea)
                if ins.size:
                    idc.MakeUnknown(ea, ins.size, idc.DOUNK_EXPAND)
                    if not idc.MakeCode(ea):
    ...
    def discover_function(self, ea):
        ...
        if symbol_name and name and symbol_name != name and not re.match("loc_", symbol_name):
            ...
            idc.DelFunction(idc.LocByName(name))
            name = None
        ...
        seg_name = idc.SegName(ea)
        if self.code_discovery and not self.is_system_lib(seg_name) and (
                not idc.isCode(idc.GetFlags(ea)) or not self.isCode):
            # print "need_hooking :: ea: %x, seg_name: %s" % (ea, seg_name)
            need_hooking = True

        idc.refresh_debugger_memory()  # need to call this here (thx IlfakG)
        # this should normally work for dynamic libraries

        r = idc.MakeFunction(ea)
        if not r:
    ...
    def dbg_step_into(self):
        ...
        if hasattr(self, 'current_caller') and self.current_caller and self.current_caller['type'] == 'jump':
            ...
            if hasattr(self, 'current_caller') and self.current_caller and ret_addr == self.next_ins(
                    self.current_caller['addr']):
...
class X86CapHook(FunCapHook):
    ...
    def is_call(self, ea):
        ...
        """
        mnem = GetDisasm(ea)
        if re.match('call\s+far ptr',
                    mnem): return None  # when IDA badly identifies data as code it throws false positives - zbot example
    ...
    def get_stack_args(self, ea, depth=None, stack_offset=1):
        ...
        for arg in range(stack_offset, depth):
            value = idc.DbgDword(stack + arg * 4)
            l.append(
                {'name': "arg_%02x" % argno, 'value': value, 'deref': self.dereference(value, 2 * self.STRING_LENGTH)})
            argno = argno + 4
        return l
    ...
    def calc_ret_shift(self, ea):
        ...
        first_head = GetFunctionAttr(ea, idc.FUNCATTR_START)
    ...
    def check_stub(self, ea):
        ...
        dbytes = idc.GetManyBytes(ea, 7, use_dbg=True)
...
class AMD64CapHook(FunCapHook):
    ...
    def __init__(self, **kwargs):
        self.arch = 'amd64'
        self.bits = 64
        self.CMT_CALL_CTX = [re.compile('^RCX'), re.compile('^RDX'), re.compile('^R8'),
                             re.compile('^R9')]  # we are capturing 4 args, but it can be extended
        self.CMT_RET_SAVED_CTX = [re.compile('^RCX'), re.compile('^RDX'), re.compile('^R8'), re.compile('^R9'),
                                  re.compile('^arg')]
...
class ARM64CapHook(FunCapHook):
    ...
    def __init__(self, **kwargs):
        self.arch = 'arm64'
        self.bits = 64
        self.CMT_CALL_CTX = [re.compile('X0$'), re.compile('X1$'), re.compile('X2$'), re.compile('X3$')]
        self.CMT_RET_SAVED_CTX = [re.compile('X0$'), re.compile('X1$'), re.compile('X2$'), re.compile('X3$')]
        self.CMT_RET_CTX = [re.compile('X0$')]
        FunCapHook.__init__(self, **kwargs)
    ...
    def is_ret(self, ea):
        ...
        """
        disasm = GetDisasm(ea)
        return re.match('RET', disasm) or re.match('POP.*,PC\}', disasm) or re.match('BX(\s+)LR', disasm)
...
class Auto:
    ...
    def win_code_discovery(self):
        ...
        d.addStop(
            LocByName("ntdll_RtlExitUserProcess"))  # to be sure we add 2 possibilities, there is probably more ...
        d.on()
...
if debugger:
    ...
    if arch == 'x86':
        ...
        raise Exception("FunCap: architecture not supported")
