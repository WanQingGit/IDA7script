    ...
    def create(cfunc, arg):
        ...
        if isinstance(arg, idaapi.ctree_item_t):
            ...
            if lvar:
                ...
                return result
            if arg.citype != idaapi.VDI_EXPR:  # 不是表达式
                return None
            cexpr = arg.e
        else:
            cexpr = arg
        ...
        if cexpr.op == idaapi.cot_var:
            ...
            result = StructPtrObject(t.dstr(), cexpr.m)  # cexpr.m为offset
...
class ObjectDownwardsVisitor(ObjectVisitor):
    ...
    def visit_expr(self, cexpr):
        ...
        for obj in self._objects:
            if obj.is_target(x_cexpr):#如果是自己本身
                if self.__is_object_overwritten( y_cexpr):#x_cexpr, obj,
    ...
    def __is_object_overwritten(self, y_cexpr):#, x_cexpr, obj
...
class RecursiveObjectVisitor(ObjectVisitor):
    def __init__(self, cfunc, obj, data=None, skip_until_object=False, visited=None):
        ...
        # self._finish = None
        # self._finish_iteration = None

    def visit_expr(self, cexpr):  # 可能是多重继承中的重载，ObjectDownwardsVisitor
        return super(RecursiveObjectVisitor, self).visit_expr(cexpr)  # ObjectDownwardsVisitor
    ...
    def process(self):
        self._start()
        self._recursive_process()  # 可能重载,RecursiveObjectDownwardsVisitor,RecursiveObjectUpwardsVisitor
        self._finish()
        self.dump_scan_tree()
    ...
    def _recursive_process(self):
        self._start_iteration() #让子类实现
        super(RecursiveObjectVisitor, self).process()  # ObjectVisitor->apply_to
        self._finish_iteration()

    def _manipulate(self, cexpr, obj):
        self._check_call(cexpr) #RecursiveObjectDownwardsVisitor
        super(RecursiveObjectVisitor, self)._manipulate(cexpr, obj) #ObjectVisitor
...
class RecursiveObjectDownwardsVisitor(RecursiveObjectVisitor, ObjectDownwardsVisitor):
    ...
    def _check_call(self, cexpr):
        ...
        grandparent = self.parents.at(self.parents.size() - 2) #citem
